...
export default function Home() {
  ...
  async function loadData() {
    ...
    const volume24h = last24h.filter(s => s.token_in === 'USDC').reduce((sum, s) => sum + Number(s.amount_in), 0) + last24h.filter(s => s.token_out === 'USDC').reduce((sum, s) => sum + Number(s.amount_out), 0)
    const totalVolume = data.filter(s => s.token_in === 'USDC').reduce((sum, s) => sum + Number(s.amount_in), 0) + data.filter(s => s.token_out === 'USDC').reduce((sum, s) => sum + Number(s.amount_out), 0)
